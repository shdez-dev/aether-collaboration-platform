...
const COLS: {
  ...
  },
  {
    ...
    count: 5,
    cards: [
      { title: 'CRDT para documentos colaborativos', priority: 'HIGH', labels: ['#a855f7', '#3b82f6'], av: 'SO', avColor: '#a855f7', dueToday: true, dueColor: '#f59e0b' },
...
function MockCard({
  title, priority, labels, av, avColor, due, dueToday, dueColor, completed,
}: (typeof COLS)[0]['cards'][0]) {
  const t = useT();
  const prio = PRIORITY_COLORS[priority];
  const prioLabel =
    priority === 'HIGH'   ? t.home_mock_priority_high   :
    priority === 'MEDIUM' ? t.home_mock_priority_medium :
                            t.home_mock_priority_low;
  const dueDisplay = dueToday ? t.home_mock_due_today : due;
  ...
  return (
    <div style={{
      ...
      borderRadius: '8px', padding: '9px 10px 8px',
    }}>
      {labels.length > 0 && (
      ...
      <div className="flex items-start gap-2">
        <span className="flex-shrink-0 flex items-center justify-center rounded-[5px] mt-[1px]" style={{
          width: '16px', height: '16px',
          border: `1.5px solid ${completed ? C.green : C.text3}`,
          background: completed ? C.green : 'transparent', flexShrink: 0,
        }}>
          {completed && (
      ...
      <div className="flex items-center gap-2 mt-2 flex-wrap" style={{ minHeight: '18px' }}>
        ...
        {dueDisplay && (
          <div className="flex items-center gap-[3px]" style={{ flexShrink: 0 }}>
            ...
            <span style={{ fontSize: '10.5px', color: dueColor, fontVariantNumeric: 'tabular-nums' }}>{dueDisplay}</span>
          </div>
        )}

        <div style={{ flex: 1 }} />

        <div className="flex items-center justify-center text-[8px] font-bold text-white" style={{
          width: '16px', height: '16px', borderRadius: '4px', background: avColor, flexShrink: 0,
        }}>
          {av}
        </div>
      </div>
...
function MockList({ name, count, cards }: { name: string; count: number; cards: (typeof COLS)[0]['cards'] }) {
  return (
    <div style={{
      background: C.bg2, border: `1px solid ${C.border}`,
      borderRadius: '10px', width: '210px', flexShrink: 0,
      display: 'flex', flexDirection: 'column',
    }}>
      <div style={{
        padding: '10px 10px 10px 8px', borderBottom: `1px solid ${C.border}`,
...
function ProductShot() {
  ...
  const colNames = [
    t.home_mock_col_backlog,
    t.home_mock_col_in_progress,
    t.home_mock_col_in_review,
    t.home_mock_col_done,
  ];

  return (
    <div className="rounded-xl overflow-hidden font-mono" style={{
      ...
      <div style={{ display: 'flex', minHeight: '480px' }}>
        ...
        <aside style={{ width: '180px', flexShrink: 0, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', background: C.bg2 }}>
          ...
          <nav style={{ flex: 1, padding: '8px', overflow: 'hidden' }}>
            {/* Workspaces */}
            <div style={{ fontSize: '10px', color: C.text4, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '4px 8px 6px', fontWeight: 500 }}>
              {t.home_mock_section_workspaces}
            </div>
            {[
              ...
              <div key={ws.name} style={{
                ...
                <span style={{ fontSize: '11.5px', color: ws.active ? C.accent : C.text2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {ws.name}
                </span>
              </div>
            ))}
            ...
            {/* Projects */}
            <div style={{ fontSize: '10px', color: C.text4, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '10px 8px 6px', fontWeight: 500 }}>
              {t.home_mock_section_projects}
            </div>
            {[
              ...
              <div key={p.name} style={{
                display: 'flex', alignItems: 'center', gap: '7px', padding: '5px 8px', borderRadius: '6px', marginBottom: '2px',
                background: p.active ? C.hover : 'transparent',
              }}>
                <svg viewBox="0 0 12 10" fill="none" width="11" height="9" style={{ flexShrink: 0 }}>
                  <rect x="0.5" y="0.5" width="3" height="9" rx="1" fill={p.active ? C.accent : C.text4} opacity={p.active ? 0.8 : 0.5} />
                  <rect x="4.5" y="2.5" width="3" height="7" rx="1" fill={p.active ? C.accent : C.text4} opacity={p.active ? 0.6 : 0.35} />
                  <rect x="8.5" y="1.5" width="3" height="8" rx="1" fill={p.active ? C.accent : C.text4} opacity={p.active ? 0.4 : 0.25} />
                </svg>
                <span style={{ fontSize: '11.5px', color: p.active ? C.text : C.text3 }}>{p.name}</span>
              </div>
            ))}

            {/* Teams */}
            <div style={{ fontSize: '10px', color: C.text4, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '10px 8px 6px', fontWeight: 500 }}>
              {t.home_mock_section_teams}
            </div>
            {[
              ...
              <div key={team.name} style={{
                ...
                <svg viewBox="0 0 14 10" fill="none" stroke="currentColor" strokeWidth="1.3" width="12" height="9" style={{ flexShrink: 0, color: C.text4 }}>
                  <circle cx="5" cy="3.5" r="2.5" />
                  <circle cx="10" cy="3.5" r="2" />
                  <path d="M0.5 9.5C0.5 8 2.5 7 5 7s4.5 1 4.5 2.5" />
                  <path d="M8.5 7.5c1-.3 2 .3 2 1.5" />
                </svg>
                <span style={{ fontSize: '11.5px', color: C.text3, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {team.name}
                </span>
                <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: team.color, flexShrink: 0 }} />
              </div>
            ))}
          </nav>
        </aside>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: C.bg }}>
          ...
          {/* Kanban columns */}
          <div style={{ padding: '14px 16px', display: 'flex', gap: '10px', overflow: 'hidden', flex: 1 }}>
            {COLS.map((col, i) => (
              <MockList key={col.key} name={colNames[i]} count={col.count} cards={col.cards} />
            ))}
          </div>
        </div>
      </div>
      ...
      <nav className="sticky top-0 z-50" style={{
        ...
        <div className="max-w-[1240px] mx-auto px-8 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {!isAuthenticated && (
              <Link href="/login" className="hidden sm:inline-flex text-sm px-3 py-2 rounded-lg transition-colors" style={{ color: 'var(--home-text-2)' }}>
                {t.home_hero_cta_login}
              </Link>
            )}
            {isAuthenticated ? (
              <Link href="/dashboard" className="inline-flex items-center text-sm font-medium text-white px-3.5 py-2 rounded-lg transition-all hover:-translate-y-px" style={{ background: '#3b82f6', boxShadow: '0 4px 14px -4px rgba(59,130,246,0.4)' }}>
                {t.home_hero_cta_dashboard}
              </Link>
            ) : (
              <Link href="/register" className="inline-flex items-center text-sm font-medium text-white px-3.5 py-2 rounded-lg transition-all hover:-translate-y-px" style={{ background: '#3b82f6', boxShadow: '0 4px 14px -4px rgba(59,130,246,0.4)' }}>
      ...
      <section className="relative pt-20 pb-0">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute inset-0" style={{
            backgroundImage: `linear-gradient(var(--home-grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--home-grid-line) 1px, transparent 1px)`,
        ...
        <div className="relative z-10 max-w-[1240px] mx-auto px-8">
          ...
          >
            {isAuthenticated ? (
              <Link href="/dashboard" className="inline-flex items-center gap-2 font-medium text-white px-5 py-3 rounded-[10px] transition-all hover:-translate-y-px text-[15px]" style={{ background: '#3b82f6', boxShadow: '0 4px 16px -4px rgba(59,130,246,0.45)' }}>
                {t.home_hero_cta_dashboard}
              </Link>
            ) : (
              <>
                <Link href="/register" className="inline-flex items-center gap-2 font-medium text-white px-5 py-3 rounded-[10px] transition-all hover:-translate-y-px text-[15px]" style={{ background: '#3b82f6', boxShadow: '0 4px 16px -4px rgba(59,130,246,0.45)' }}>
                  {t.home_hero_cta_start} →
                </Link>
                <Link href="/login" className="inline-flex items-center font-medium text-[15px] px-5 py-3 rounded-[10px] transition-all" style={{ color: 'var(--home-glass-text)', background: 'var(--home-glass)', border: '1px solid var(--home-glass-border)' }}>
                  {t.home_hero_cta_login}
                </Link>
              </>
            )}
          ...
          <motion.div
            ...
            style={{ color: 'var(--home-text-3)', borderTop: '1px solid var(--home-border)' }}
          >
            {[
